...
export default function TypeDetailPage() {
  ...
  if (isLoading) {
    return (
      ...
      >
        <Skeleton className="h-8 w-32 mb-8 rounded-2xl" />
    ...
    <main className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      >
        <Link
          ...
          className="inline-flex items-center gap-2 text-foreground/70 hover:text-primary font-bold transition-colors mb-8 font-body"
        >
      ...
      <motion.div
        ...
        className={`card-regal p-8 bg-${catClass} mb-8 relative overflow-hidden`}
      >
        ...
        <div className="relative z-10">
          <Badge
            variant="outline"
            className={`category-${catClass} border-current mb-4 border-2 font-body`}
          >
            {lang === "en" ? type.category : CATEGORY_HI[type.category]}
          </Badge>
          <h1
            className="font-display text-6xl md:text-7xl text-primary mb-2"
            style={{ textShadow: "3px 3px 0 oklch(15 0.02 265 / 0.4)" }}
          >
            {type.code}
          </h1>
      ...
      <motion.div
        ...
        className="card-regal p-6 mb-6"
      >
        <h3 className="font-display text-xl text-primary mb-3">
          {t("About this Type", "इस प्रकार के बारे में")}
        </h3>
        <p className="text-foreground/85 leading-relaxed text-base font-body">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        >
          ...
          <ul className="space-y-2">
            {strengths.map((s) => (
              <li
                key={s}
                className="flex items-center gap-2 text-foreground/85 font-body"
              >
                <span className="w-3 h-3 rounded-full border-2 border-green-600 bg-green-400 flex-shrink-0" />
                {s}
              </li>
            ))}
          </ul>
        ...
        <motion.div
          ...
          className="card-regal p-6"
          style={{ background: "oklch(93 0.09 85)" }}
        >
          <h3
            className="font-display text-xl mb-4 flex items-center gap-2"
            style={{ color: "oklch(45 0.22 55)" }}
          >
          ...
          <ul className="space-y-2">
            {weaknesses.map((w) => (
              <li
                ...
                className="flex items-center gap-2 text-foreground/85 font-body"
              >
                <span className="w-3 h-3 rounded-full border-2 border-amber-600 bg-amber-400 flex-shrink-0" />
                {w}
              </li>
            ))}
          </ul>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <motion.div
          ...
          className="card-regal p-6"
          style={{ background: "oklch(94 0.07 340)" }}
        >
          <h3
            className="font-display text-xl mb-4 flex items-center gap-2"
            style={{ color: "oklch(45 0.25 340)" }}
          >
          ...
          <ul className="space-y-2">
            {likes.map((item) => (
              <li
                ...
                className="flex items-center gap-2 text-foreground/85 font-body"
              >
                <span className="w-3 h-3 rounded-full border-2 border-pink-600 bg-pink-400 flex-shrink-0" />
                {item}
              </li>
            ))}
          </ul>
        ...
        <motion.div
          ...
          className="card-regal p-6"
          style={{ background: "oklch(93 0.07 25)" }}
        >
          <h3
            className="font-display text-xl mb-4 flex items-center gap-2"
            style={{ color: "oklch(40 0.22 25)" }}
          >
          ...
          <ul className="space-y-2">
            {dislikes.map((item) => (
              <li
                ...
                className="flex items-center gap-2 text-foreground/85 font-body"
              >
                <span className="w-3 h-3 rounded-full border-2 border-red-600 bg-red-400 flex-shrink-0" />
                {item}
              </li>
            ))}
          </ul>
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          {famous.map((name) => (
            <span
              key={name}
              className="font-body text-sm py-1.5 px-3 rounded-full border-2 border-purple-400 bg-purple-100 text-purple-800 font-medium"
              style={{ boxShadow: "2px 2px 0 oklch(40 0.25 305)" }}
            >
              {name}
            </span>
          ))}
      ...
      <div className="flex gap-4">
        <Link to="/types" search={{ category: undefined }}>
          <button
            type="button"
            className="btn-cartoon bg-secondary text-white font-display px-5 py-2 rounded-2xl text-sm"
        ...
        <Link to="/quiz">
          <button
            type="button"
            className="btn-cartoon bg-primary text-white font-display px-5 py-2 rounded-2xl text-sm"
